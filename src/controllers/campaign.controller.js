...
export const createCampaign = async (req, res) => {
    try {
        ...
        const body = req.body;
        const campaignRef = doc(db, 'campaign', newId).withConverter(addCreatedTimestamps);
        await setDoc(campaignRef, body);
        res.status(201).json({ message: "Campaign created successfully", id: campaignRef.id });
    } catch (error) {
...
export const updateCampaign = async (req, res) => {
    try {
        const campaignId = req.params["id"];
        const upRef = doc(db, "campaign", campaignId).withConverter(addCreatedTimestamps);
        const body = req.body;
        ...
        res.status(200).json({ message: "Campaign updated successfully", id: campaignId });
    } catch (error) {
...
export const deleteCampaign = async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Campaign deleted successfully", id: campaignId });
    } catch (error) {
